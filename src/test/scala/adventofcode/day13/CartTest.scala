...
class CartTest extends WordSpec with Matchers {
  "moveTo" must {
    "stay straight on WE when heading E or W" in {
      val track = '-'
      Cart(Direction.West).moveTo(track) shouldBe Cart(Direction.West)
      Cart(Direction.East).moveTo(track) shouldBe Cart(Direction.East)
    }

    "stay straight on NS when heading N or S" in {
      val track = '|'
      Cart(Direction.North).moveTo(track) shouldBe Cart(Direction.North)
      Cart(Direction.South).moveTo(track) shouldBe Cart(Direction.South)
    }

    "for /" must {
      "turn right when heading north" in {
        Cart(Direction.North).moveTo('/') shouldBe Cart(Direction.East)
      }

      "turn left when heading west" in {
        Cart(Direction.West).moveTo('/') shouldBe Cart(Direction.South)
      }

      "turn right when heading south" in {
        Cart(Direction.South).moveTo('/') shouldBe Cart(Direction.West)
      }

      "turn left when heading east" in {
        Cart(Direction.East).moveTo('/') shouldBe Cart(Direction.North)
      }
    }

    "for backslash" must {
      "turn left when heading north" in {
        Cart(Direction.North).moveTo('\\') shouldBe Cart(Direction.West)
      }

      "turn right when heading west" in {
        Cart(Direction.West).moveTo('\\') shouldBe Cart(Direction.North)
      }

      "turn left when heading south" in {
        Cart(Direction.South).moveTo('\\') shouldBe Cart(Direction.East)
      }

      "turn right when heading east" in {
        Cart(Direction.East).moveTo('\\') shouldBe Cart(Direction.South)
      }
    }
